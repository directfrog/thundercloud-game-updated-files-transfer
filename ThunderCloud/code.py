...
tile_structures = ['a', 'b', 'c', 'd']
structures = ['e', 'f', 'l', 'x', 'y', 'p', 'g']
entities = ['F']
...
while True:
		...
		for x, tile in enumerate(row):
			...
			if tile in structures:
				if (x*32-scroll[0] < render_box.x+screen_width+64) and (x*32-scroll[0] > render_box.x-130) and (y*32-scroll[1] < render_box.y+screen_height+160) and (y*32-scroll[1] > render_box.y-64):
					...
					if tile == 'f':
						screen.blit(big_house, (x*32-scroll[0]-32, y*32-scroll[1]-136))
	...
	##### Wind Particles #####
	if len(wind_particles) < 5:
		if random.randint(1, 4) == 3:
			for _ in range(2): ##### This number will affect the amount of wind
	...
	#	# draw farmers onto screen
	#	screen.blit(farmer, (farmer[0], farmer[1]))
