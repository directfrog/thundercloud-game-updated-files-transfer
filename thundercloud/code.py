...
def main_game():
	...
	guardian_attack.set_colorkey((255, 255, 255))
	guardian_attack = pygame.transform.scale(guardian_attack, (guardian_attack.get_width()*guardian_scale, guardian_attack.get_height()*guardian_scale))	
	#if level2:
	#	guardian = get_guardians(secondscene)
	#	current_frame = guardian_walktick += 1
	...
	non_tile_objects = ['0', 'P', 'C', 'S', 'T', '?', '@', ';', ':', 'K', 'N', 'M', 'R', 'G']
	...
	while True:
		...
		##### Setting up Guardian #####
		if level2:
			if abs(guardian[0]-player_rect.x) < 300:
				if guardian_walking:
